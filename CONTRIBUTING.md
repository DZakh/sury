...
- A `Blob` or `File` can't be a recorded *result*, because reading its bytes back out is async and the serializer is sync ("cannot represent a Blob instance as spec source code"). Passing examples for `S.file`/`S.blob` and the size bounds therefore can't be written at all — `specs/file*.yaml` and `specs/blob*.yaml` cover only the rejecting cases, and `specs/set-minSize.yaml` carries the pass/fail pair for the shared `.size` path using a `Set`, which does serialize. An async serialization pass, or an opt-in "assert it did not throw" example form, would close it.
