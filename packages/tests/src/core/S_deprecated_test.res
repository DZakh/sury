...
test("Set deprecation", t => {
  ...
  let withDeprecation = original->S.deprecated("Use number instead")

  t->Assert.deepEqual((withDeprecation->S.untag).deprecated, Some("Use number instead"), ())
  t->Assert.deepEqual(
...
test("Transforms don't remove deprecation", t => {
  let schema = S.string->S.deprecated("Original schema is not mutated.")->S.trim

  t->Assert.deepEqual((schema->S.untag).deprecated, Some("Original schema is not mutated."), ())
})

test("Deprecated is a metadata only and doesn't make the field optional", t => {
  let schema = S.string->S.deprecated("Use number instead.")
