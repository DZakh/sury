...
Tells to use `S.nullish` for the option schema constructor.

```rescript
@schema
type t = @s.nullish option<string>
...
It might be used together with `@s.null` or `@s.nullish`:

```rescript
@schema
type t = @s.nullish @s.default("Unknown") string

// Generated by PPX ⬇️
let schema = S.nullish(S.string)->S.Option.getOr("Unknown")
```
...
> 🧠 The same as `@s.default` it might be used together with `@s.null` or `@s.nullish`
...
```rescript
@schema
type t = @s.description("A useful bit of text, if you know what to do with it.") string

// Generated by PPX ⬇️
let schema = S.string->S.description("A useful bit of text, if you know what to do with it.")
```
...
// Generated by PPX ⬇️
let schema = S.string->S.deprecated("Will be removed in APIv2")
```
