...
module CknittelBugReport2 = {
  ...
  test("Successfully parses nested optional union", t => {
    ...
    t->Assert.deepEqual(S.convertOrThrow("{}", ~from=S.jsonString, ~to=schema), {test: None})
  })
  ...
  test("Nested literal field with catch", t => {
    ...
    t->Assert.deepEqual(S.convertOrThrow(`{"statusCode": {"kind": "ok"}}`, ~from=S.jsonString, ~to=schema), Ok())
  })
}
