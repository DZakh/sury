...
export const scaffoldOperations = (schema: any): Spec["operations"] => {
  ...
  return Object.fromEntries(
    OP_ORDER.map((opName) => {
      const fn = OP_BUILDER[opName](schema);
      // compilePerf (optional) is filled by the perf pass on `spec check
      // --write`; a fresh scaffold omits it.
...
export const serialize = (obj: Spec): string =>
  // aliasDuplicateObjects: false — specs are golden data files; an incidentally
  // repeated value must serialize as plain repeated YAML, never a surprising
  // anchor/alias (`&a1`/`*a1`) that would hurt readability and diffs.
