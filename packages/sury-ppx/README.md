...
Represents `None` as `null` instead of `undefined`.
...
Parses both `null` and `undefined` to `None`, and turns `None` back into `undefined`.
...
On an optional record field it replaces `S.option`, so a missing key and `null` both parse to `None`:
