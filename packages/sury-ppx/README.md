...
Instead of a lambda, the payload can be a tuple of a transform function and arguments to pass after the schema, mirroring the JS `schema.with(fn, ...args)`. ReScript attribute payloads must be a single expression, hence the tuple parentheses — the partial-application placeholder is an equivalent spelling without them:

```rescript
@schema
type t = @s.with((S.min, 1)) int
// or
type t2 = @s.with(S.min(_, 1)) int
...
```rescript
@schema
type t = @s.with(S.trim) @s.with((S.min, 1)) @s.with(s => s->S.max(5)) string
