...
It also works on optional record fields, where `S.nullAsOption` is used instead of `S.option` for the field optionality. The `?` keeps the field optional in ReScript, while the data must still have the key set to `null`:
