...
For transforms taking arguments after the schema, use the partial-application placeholder — it compiles to a direct call with no intermediate lambda:

```rescript
@schema
type t = @s.with(S.min(_, 1)) int

// Generated by PPX ⬇️ (simplified)
let schema = S.int->S.min(1)
```

Unlike other attributes, `@s.with` may be used multiple times on the same type — the transforms are applied in order of appearance:

```rescript
@schema
type t = @s.with(S.trim) @s.with(S.min(_, 1)) @s.with(s => s->S.max(5)) string

// Generated by PPX ⬇️ (simplified)
let schema = S.string->S.trim->S.min(1)->S.max(5)
```
...
`@s.with` on an outer type composes with attributes on its type arguments, which are resolved first:

```rescript
@schema
type t = @s.with(s => s->S.meta({description: "items"})) array<@s.default("x") string>

// Generated by PPX ⬇️ (simplified)
let schema = S.array(S.option(S.string)->S.Option.getOr("x"))->S.meta({description: "items"})
```
