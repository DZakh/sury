...
// `sury/wkt`: the well-known protobuf types, generated by protoc-gen-sury into
// src/wkt. It imports the library through "sury", which stays external, so the
// schemas it builds are the same instance's that index.mjs parses with.
async function buildWkt(format: "esm" | "cjs", outfile: string): Promise<void> {
  await build({
    entryPoints: [path.join(projectPath, "src/wkt/index.ts")],
    outfile,
    bundle: true,
    write: true,
    format,
    ...
    platform: "neutral",
    external: ["sury"],
    banner: { js: "// Generated from src/wkt by scripts/pack.ts - do not edit." },
    logLevel: "silent",
  });
}

const buildDevEntries = async (): Promise<void> => {
  await buildEntry("esm", path.join(projectPath, "index.mjs"));
  await buildWkt("esm", path.join(projectPath, "wkt.mjs"));
};
