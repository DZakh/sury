...
module Common = {
  ...
  test("Fails to serialize invalid value", t => {
    ...
    t->U.assertThrowsMessage(
      () => invalidValue->S.convertOrThrow(~from=schema, ~to=S.unknown),
      `Expected "ReScript is Great!", received "Hello world!"`,
    )
  })
  ...
  test("Compiled parse code snapshot", t => {
    ...
    t->U.assertCompiledCode(~schema, ~op=#Parse, `i=>{try{i==="ReScript is Great!"||e[0](i);return i}catch(v0){e[1](v0)}}`)
  })

  test("Compiled serialize code snapshot", t => {
    let schema = factory()

    t->U.assertCompiledCode(~schema, ~op=#Encode, `i=>{try{i==="ReScript is Great!"||e[0](i);return i}catch(v0){e[1](v0)}}`)
  })
