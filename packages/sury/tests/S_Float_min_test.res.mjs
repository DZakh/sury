// Generated by ReScript, PLEASE EDIT WITH CARE

import * as S from "../src/S.res.mjs";
import * as U from "./U.res.mjs";
import Ava from "ava";

Ava("Successfully parses valid data", (function (t) {
        var schema = S.floatMin(S.$$float, 1, undefined);
        t.deepEqual(S.parseOrThrow(1, schema), 1, undefined);
        t.deepEqual(S.parseOrThrow(1234, schema), 1234, undefined);
      }));

Ava("Fails to parse invalid data", (function (t) {
        var schema = S.floatMin(S.$$float, 1, undefined);
        U.assertThrows(t, (function () {
                return S.parseOrThrow(0, schema);
              }), {
              operation: "Parse",
              code: {
                TAG: "OperationFailed",
                _0: "Number must be greater than or equal to 1"
              },
              path: S.Path.empty
            });
      }));

Ava("Successfully serializes valid value", (function (t) {
        var schema = S.floatMin(S.$$float, 1, undefined);
        t.deepEqual(S.reverseConvertOrThrow(1, schema), 1, undefined);
        t.deepEqual(S.reverseConvertOrThrow(1234, schema), 1234, undefined);
      }));

Ava("Fails to serialize invalid value", (function (t) {
        var schema = S.floatMin(S.$$float, 1, undefined);
        U.assertThrows(t, (function () {
                return S.reverseConvertOrThrow(0, schema);
              }), {
              operation: "ReverseConvert",
              code: {
                TAG: "OperationFailed",
                _0: "Number must be greater than or equal to 1"
              },
              path: S.Path.empty
            });
      }));

Ava("Returns custom error message", (function (t) {
        var schema = S.floatMin(S.$$float, 1, "Custom");
        U.assertThrows(t, (function () {
                return S.parseOrThrow(0, schema);
              }), {
              operation: "Parse",
              code: {
                TAG: "OperationFailed",
                _0: "Custom"
              },
              path: S.Path.empty
            });
      }));

Ava("Returns refinement", (function (t) {
        var schema = S.floatMin(S.$$float, 1, undefined);
        t.deepEqual(S.Float.refinements(schema), [{
                kind: {
                  TAG: "Min",
                  value: 1
                },
                message: "Number must be greater than or equal to 1"
              }], undefined);
      }));

export {
  
}
/*  Not a pure module */
