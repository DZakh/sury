...
test("Successfully parses valid data", t => {
  ...
  t->Assert.deepEqual("2020-01-01T00:00:00Z"->S.parseOrThrow(schema), "2020-01-01T00:00:00Z")
  t->Assert.deepEqual(
...
test("Successfully serializes valid value", t => {
  ...
  t->Assert.deepEqual(
    "2020-01-01T00:00:00.123Z"->S.reverseConvertOrThrow(schema),
...
test("Can be combined with S.to(S.date) for string-to-Date decoding", t => {
  ...
  t->U.assertThrowsMessage(
    () => "not-a-date"->S.parseOrThrow(schema),
    `Invalid datetime string! Expected UTC`,
  )
})
