...
test("Successfully parses", t => {
  ...
  t->Assert.deepEqual("true"->S.convertOrThrow(~from=S.jsonString, ~to=schema), true)
})
...
test("Successfully parses unknown", t => {
  ...
  t->Assert.deepEqual(
    "true"->S.convertOrThrow(~from=S.jsonString, ~to=schema),
    "true"->Obj.magic,
    ~message="S.unknown should keep json schema as a value",
  )

  t->Assert.deepEqual(
    "tru"->S.convertOrThrow(~from=S.jsonString, ~to=schema),
    "tru"->Obj.magic,
...
test("Fails to parse JSON", t => {
  ...
  U.assertThrowsMessage(
    t,
    () => "123,"->S.convertOrThrow(~from=S.jsonString, ~to=schema),
