...
test("Coerce from string to option of int (union dispatch over a converted value)", t => {
  ...
  // a self-contained codeFromPrev unit, so the int branch keeps that
  // declaration inside the direct-dispatch arm.
  t->U.assertCompiledCode(
    ~schema,
    ~op=#Parse,
...
test("Refined+converted target union is still an ambiguous widening", t => {
  ...
  t->U.assertCompiledCode(
    ~schema=explicit,
    ~op=#Parse,
    `i=>{typeof i==="string"||e[3](i);e[0](i)||e[2](i);let v0;try{v0=BigInt(i)}catch(_){e[1](i)}i=v0;return i}`,
  )
})
