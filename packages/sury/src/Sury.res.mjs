...
function shrunkenWiderUnionSchemas(schemaAnyOf, inputAnyOf) {
  let shrunken = [];
  let allMatch = inputAnyOf.every(inputSchema => {
    if (flags[inputSchema.type] & 9152 || inputSchema.to) {
      return false;
    }
    let matched = schemaAnyOf.some(targetSchema => {
      if (inputSchema.type === targetSchema.type) {
        return inputSchema.const === targetSchema.const;
      } else {
        return false;
      }
    });
    if (matched) {
      shrunken.push(inputSchema);
    }
    return matched;
  });
  if (allMatch) {
...
function unionDecoder(input) {
  ...
  let shrunkenPassthrough = initialInputTagFlag & 256 && toPerCase === undefined ? shrunkenWiderUnionSchemas(schemas, input.s.anyOf) : undefined;
  if (shrunkenPassthrough || input.io && input.e === input.s) {
    if (shrunkenPassthrough !== undefined && shrunkenPassthrough.length !== input.s.anyOf.length) {
      input.s = factory$1(shrunkenPassthrough);
    }
    return input;
  }
