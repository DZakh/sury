...
function arrayDecoder(unknownInput) {
  ...
  if (itemSchema === "strip" || itemSchema === "strict") {
    ...
    let output$1 = hasTransform ? next(input, `new Array(` + inputVar + `.length)`, array(itemOutput.s), undefined) : refine(input, expectedSchema, undefined, undefined);
...
function objectDecoder(unknownInput) {
  ...
  if (dictItem !== undefined) {
    if (dictItem === unknown) {
      ...
      let itemOutput = parseDynamic(itemInput);
      let hasTransform = itemOutput.t;
      let output$1 = hasTransform ? next(input, "{}", dictFactory(itemOutput.s), undefined) : refine(input, expectedSchema, undefined, undefined);
        ...
        try {
          ...
          return {
            issues: [{
                ...
                path: error.path === "" ? undefined : toArray(error.path).map(key => (key))
              }]
          };
        }
      },
    ...
    if (anyOf !== undefined) {
      ...
      if ($$default.TAG === "Value") {
        ...
        try {
          ...
          let error = getOrRethrow(exn);
          let message$1 = `Invalid default for ` + toExpression(mut) + `: ` + error.message;
          throw new Error(`[Sury] ` + message$1);
        }
        try {
          mut.default = getDecoder(reverse(originalItem), undefined)(v);
        } catch (exn$1) {
      ...
      to.serializer = input => {
        let nextSchema = reverse(originalItem);
...
function jsonStringEncoder(input, target) {
  ...
  output.v = _var;
  let inputVar = input.v();
  output.cp = `let ` + outputVar + `;try{` + outputVar + `=JSON.parse(` + inputVar + `)}catch(t){` + embedInvalidInput(input, input.s) + `}`;
  return output;
}
...
function list(schema) {
  return transform(array(schema), param => ({
    p: Stdlib_List.fromArray,
    s: Stdlib_List.toArray
  }));
}
...
function compactColumnsDecoder(input) {
  ...
  if (maybeProperties !== undefined) {
    ...
    if (forwardProps) {
      ...
      output$1.v = _var;
      output$1.cp = `let ` + outputVar + `=new Array(Math.max(` + lengthCode + `));`;
      let rowAssign;
      if (hasAsync) {
    ...
    output$3.v = _var;
    output$3.cp = `let ` + outputVar$1 + `=[` + initialArraysCode + `];`;
...
function customBuilder(fn) {
  return input => {
    ...
    output.v = _var;
    output.cp = `let ` + outputVar + `;try{` + output.i + `=` + embed(input, fn) + `(` + input.i + `)}catch(x){` + failWithArg(output, e => makeInvalidConversionDetails(input, target, e), `x`) + `}`;
    return output;
  };
}
...
function js_merge(s1, s2) {
  ...
  if (s1.type === "object" && s2.type === "object") {
    ...
    if (typeof additionalItems1 === stringTag && typeof s2.additionalItems === stringTag && !s1.to && !s2.to) {
    ...
    switch (schema.type) {
      ...
      case "union" :
        ...
        schema.anyOf.forEach(childSchema => {
          ...
          let childJsonSchema = internalToJSONSchema(childSchema, path, defs, schema, target);
...
function toJSONSchema(schema, options) {
  ...
  let schemaUri = match[1];
  let target$2 = match[0];
  let defs = {};
  let jsonSchema = internalToJSONSchema(schema, "", defs, schema, target$2);
  ((delete defs.JSON));
  let defsKeys = Object.keys(defs);
  if (defsKeys.length) {
    defsKeys.forEach(key => {
      let schema = defs[key];
      defs[key] = internalToJSONSchema(schema, "", 0, schema, target$2);
    });
...
function fromJSONSchema(jsonSchema) {
  ...
  if (jsonSchema.nullable) {
    ...
    if (type_$1 === "object") {
      ...
      let prefixItems = jsonSchema.prefixItems;
      let schema$2;
      if (prefixItems !== undefined) {
        schema$2 = tuple(s => prefixItems.map((d, idx) => s.item(idx, definitionToSchema$1(d))));
      } else {
