...
function unionDecoder(input) {
  ...
  if (input.s.encoder === undefined && initialInputTagFlag & 256 && input.s !== selfSchema && !schemas.some(s => s.type === refTag)) {
    let sourceCases = input.s.anyOf;
    ...
    let nextElse = false;
    let noopChecks = "";
    let fail = () => embed(input, function () {
      let args = arguments;
      let errorDetails = makeInvalidInputDetails(selfSchema, unknown, input.path, args[0], true, undefined, undefined);
      throw new SuryError(errorDetails);
    }) + "(" + input.v() + ")";
    for (let srcIdx = 0, srcIdx_finish = sourceCases.length; srcIdx < srcIdx_finish; ++srcIdx) {
      let srcCase = sourceCases[srcIdx];
      let tagFlag = flags[srcCase.type];
          ...
          tagFlag & 16 ? unit() : (
              tagFlag & 64 ? factory(unknown) : (
                  tagFlag & 128 ? array(unknown) : (
                      tagFlag & 8192 ? instance(srcCase.class) : (
                          tagFlag & 2048 ? nan() : (
                              tagFlag & 2 ? string() : (
                                  tagFlag & 4 ? float() : (
                                      tagFlag & 8 ? bool() : (
      ...
      let caseInput = refine(typeValidationOutput, srcCase, undefined, selfSchema);
      caseInput.io = false;
      caseInput.t = false;
      let caseOutput;
      try {
        caseOutput = unionDecoder(caseInput);
      } catch (exn$1) {
        caseOutput = caseInput;
      }
      outputAnyOf.push(caseOutput.s);
      let caseCond = {
        contents: ""
      };
      let caseCode = merge(caseOutput, caseCond);
      let caseCond$1 = caseCond.contents;
      if (caseOutput.t) {
        output.t = true;
        if (caseOutput.f & 1) {
          output.f = output.f | 1;
      ...
      let isIdentity = !caseOutput.t && caseOutput.i === initialInline && !(caseOutput.f & 1);
      if (isIdentity) {
        if (caseCond$1) {
          noopChecks = noopChecks ? noopChecks + "||" + caseCond$1 : caseCond$1;
        }
        
      } else {
        let body = caseOutput.i !== initialInline ? caseCode + (input.v() + "=" + caseOutput.i + ";") : caseCode;
        let if_ = nextElse ? "else if" : "if";
        let cond = caseCond$1 ? caseCond$1 : "true";
        dispatchCode = dispatchCode + if_ + ("(" + cond + "){" + body + "}");
        nextElse = true;
      }
    }
    if (noopChecks) {
      let if_$1 = nextElse ? "else if" : "if";
      dispatchCode = dispatchCode + if_$1 + ("(!(" + noopChecks + ")){" + fail() + "}");
    } else if (nextElse) {
      dispatchCode = dispatchCode + ("else{" + fail() + "}");
    }
    output.cp = output.cp + dispatchCode;
    if (input.i !== output.i) {
      output.i = input.i;
    }
    output.s = outputAnyOf.length ? factory$1(outputAnyOf) : never_();
    output.e = toPerCase !== undefined ? (output.io = true, getOutputSchema(toPerCase)) : selfSchema;
    return output;
  }
  ...
  for (let idx = 0; idx <= lastIdx; ++idx) {
    ...
    let tag = schema.type;
    let tagFlag$1 = flags[tag];
    let key = toKey(schema);
    if ((activeKey$1 === "" || activeKey$1 === key) && !(tagFlag$1 & 16 && "fromDefault" in selfSchema)) {
      let initialArr = byKey[key];
      if (initialArr !== undefined) {
        if (tagFlag$1 & 64 && nestedLoc in schema.properties) {
          initialArr.splice(initialArr.length - 1 | 0, 0, schema);
        } else if (!(tagFlag$1 & 2096)) {
          initialArr.push(schema);
        }
        
      } else {
        let typeValidationInput$1 = scope(input);
        typeValidationInput$1.e = tagFlag$1 & 32 ? nullLiteral() : (
            tagFlag$1 & 16 ? unit() : (
                tagFlag$1 & 64 ? factory(unknown) : (
                    tagFlag$1 & 128 ? array(unknown) : (
                        tagFlag$1 & 8192 ? instance(schema.class) : (
                            tagFlag$1 & 2048 ? nan() : (
                                tagFlag$1 & 2 ? string() : (
                                    tagFlag$1 & 4 ? float() : (
                                        tagFlag$1 & 8 ? bool() : (
                                            tagFlag$1 & 1024 ? bigint() : (
                                                tagFlag$1 & 16384 ? symbol() : unknown
                                              )
                                          )
                                      )
                                  )
        ...
        if (shouldDeopt) {
          for (let keyIdx = 0, keyIdx_finish = keys.length; keyIdx < keyIdx_finish; ++keyIdx) {
            ...
            if (!exit) {
              ...
              let blockCode = merge(typeValidationOutput$2, undefined) + itemsCode;
              if (blockCode) {
  ...
  if (!exit) {
    ...
    for (let idx$1 = 0, idx_finish = keys$1.length; idx$1 < idx_finish; ++idx$1) {
      ...
      let blockCode$1 = merge(typeValidationOutput$3, blockCond) + itemsCode$1;
      ...
      if (blockCode$1 || isPriority(flags[firstSchema.type], byKey$1)) {
        ...
        start = start + if_$2 + ("(" + blockCond$1 + "){" + blockCode$1 + "}");
        nextElse$1 = true;
      } else {
    ...
    if (noop) {
      ...
      tmp = if_$3 + ("(!(" + noop + ")){" + errorCode + "}");
    } else {
      tmp = nextElse$1 ? "else{" + errorCode + "}" : errorCode;
    }
    start = start + tmp;
  }
  ...
  let o = output$1.f & 1 ? (output$1.i = "Promise.resolve(" + output$1.i + ")", output$1.v = _notVar, output$1) : (
  ...
  o.s = outputAnyOf$1.length ? factory$1(outputAnyOf$1) : never_();
...
function jsonEncoderFn(input, target) {
...
function jsonString() {
  return cached("json", stringTag, s => {
    s.format = "json";
...
function uint8Array() {
  return cached("u", instanceTag, s => {
    ...
    s.decoder = inputArg => {
      ...
      let input = inputArg;
      if (inputTagFlag & 2) {
        input = next(input, embed(input, (new TextEncoder())) + ".encode(" + input.i + ")", s, undefined);
...
function isoDateTime() {
  return cached("date-time", stringTag, s => {
    ...
    s.refiner = input => [{
        ...
        f: failWithErrorMessage("format", "Invalid datetime string! Expected UTC")
      }];
  });
}
...
function port() {
  return cached("port", numberTag, s => {
    ...
    s.refiner = param => [{
        ...
        f: failWithErrorMessage("format", undefined)
      }];
  });
}
...
function date() {
  return cached(instanceTag, instanceTag, s => {
    ...
    s.decoder = input => {
      let inputTagFlag = flags[input.s.type];
      if (inputTagFlag & 2) {
        return invalidDateRefine(next(input, "new Date(" + input.i + ")", s, undefined));
  ...
  return internalRefine(schema, mut => {
    ...
    return input => {
      ...
      return [{
          ...
          f: failWithErrorMessage("pattern", message)
        }];
    };
  });
}
...
function toIntSchema(jsonSchema) {
  ...
  if (minimum !== undefined) {
    schema$1 = intMin(schema, minimum | 0, undefined);
  } else {
    let exclusiveMinimum = jsonSchema.exclusiveMinimum;
    schema$1 = exclusiveMinimum !== undefined ? intMin(schema, exclusiveMinimum + 1 | 0, undefined) : schema;
  }
  let maximum = jsonSchema.maximum;
  if (maximum !== undefined) {
    return intMax(schema$1, maximum | 0, undefined);
  }
  let exclusiveMinimum$1 = jsonSchema.exclusiveMinimum;
  if (exclusiveMinimum$1 !== undefined) {
    return intMax(schema$1, exclusiveMinimum$1 - 1 | 0, undefined);
  } else {
    return schema$1;
  }
}
...
function fromJSONSchema(jsonSchema) {
  ...
  if (exit$1 === 2) {
    ...
    if (definitions$1 !== undefined) {
      ...
      schema = len$1 !== 1 ? (
          len$1 !== 0 ? refine$1(anySchema, data => definitions.every(d => {
              try {
            ...
            }), "Should pass for all schemas of the allOf property.", undefined) : anySchema
      ...
      if (definitions$2 !== undefined) {
        ...
        schema = len$2 !== 1 ? (
            len$2 !== 0 ? refine$1(anySchema, data => {
                let validCount = {
                  contents: 0
                };
              ...
              }, "Should pass exactly one schema according to the oneOf property.", undefined) : anySchema
          ) : definitionToSchema$1(definitions$2[0]);
      } else {
        let not = jsonSchema.not;
        if (not !== undefined) {
          schema = refine$1(anySchema, data => {
            try {
          ...
          if ($$const !== undefined) {
            ...
            if (exit$2 === 3) {
              if (type_$2 === "number") {
                ...
                let schema$8;
                if (minimum !== undefined) {
                  schema$8 = floatMin(schema$7, minimum, undefined);
                } else {
                  let exclusiveMinimum = jsonSchema.exclusiveMinimum;
                  schema$8 = exclusiveMinimum !== undefined ? floatMin(schema$7, exclusiveMinimum + 1, undefined) : schema$7;
                }
                let maximum = jsonSchema.maximum;
                if (maximum !== undefined) {
                  schema = floatMax(schema$8, maximum, undefined);
                } else {
                  let exclusiveMinimum$1 = jsonSchema.exclusiveMinimum;
                  schema = exclusiveMinimum$1 !== undefined ? floatMax(schema$8, exclusiveMinimum$1 - 1, undefined) : schema$8;
                }
  ...
  switch (schema.type) {
    ...
    case "array" :
      ...
      return internalRefine(schema, mut => {
        ...
        return param => [{
            c: inputVar => inputVar + ".length===" + length$1,
            f: failWithErrorMessage("minItems", message$1)
          }];
      });
    default:
