    ...
    default:
      ...
      switch (tag) {
        ...
        case "array" :
          ...
          if (typeof additionalItems$2 !== "object") {
...
function reverse(schema) {
  ...
  while (current) {
    ...
    let items = mut.items;
    if (items !== undefined) {
      mut.items = Stdlib_Array.fromInitializer(items.length, idx => reverse(items[idx]));
    }
...
function getShapedSerializerOutput(input, acc, targetSchema, path) {
  ...
  if (exit === 1) {
    ...
    let exit$1 = 0;
    let exit$2 = 0;
    if (items !== undefined && (acc !== undefined || typeof resolvedTargetSchema.additionalItems !== "object")) {
      for (let idx = 0, idx_finish = items.length; idx < idx_finish; ++idx) {
    ...
    if (exit$2 === 3) {
      ...
      if (properties$1 !== undefined && (acc !== undefined || typeof resolvedTargetSchema.additionalItems !== "object")) {
...
function object(definer) {
  ...
  let flatten = schema => {
    ...
    if (match === "object") {
      ...
      return proxifyShapedSchema(schema, immutableEmpty$1, (f.push(schema), f.length - 1 | 0));
    }
