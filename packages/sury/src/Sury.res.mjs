...
function merge(target, subObjectVal) {
  ...
  for (let idx = (subObjectVal.j ? 7 : 4), idx_finish = inlinedLocations.length; idx < idx_finish; ++idx) {
    ...
    default:
      ...
      switch (tag) {
        case "instance" :
          ...
          let c = inputVar + " instanceof " + embed(b, $$class);
          if (negative) {
            return "!(" + c + ")";
          } else {
            return c;
...
function withCoerceScope(b, input, path, target, coercion) {
  let inputVar = input.v(b);
  return coercion(b, inputVar, failWithArg(b, path, input => ({
    TAG: "InvalidType",
    expected: target,
    received: input
  }), inputVar));
...
function parse(prevB, schema, inputArg, path) {
  ...
  if (to !== undefined) {
    ...
    if (parser !== undefined) {
      ...
      let target = schema.to;
      let isFromLiteral = isLiteral(schema);
      let isTargetLiteral = isLiteral(target);
      let exit = 0;
      let fromTag = schema.type;
      let exit$1 = 0;
      let exit$2 = 0;
      let targetTag = target.type;
      if (isFromLiteral && isTargetLiteral) {
        ...
        switch (fromTag) {
          case "string" :
            let match = target.type;
            let exit$3 = 0;
            switch (match) {
              case "string" :
                let match$1 = target.const;
                if (match$1 !== undefined) {
                  exit = 1;
                }
                break;
            ...
            if (exit$3 === 4) {
              ...
              if (isTargetLiteral) {
                input = withCoerceScope(b, input, path, target, (b, inputVar, failCoercion) => {
          ...
          case "number" :
            let match$2 = schema.format;
            if (match$2 === "int32") {
              let match$3 = target.type;
              if (match$3 === "number") {
                let match$4 = target.format;
      ...
      if (exit$1 === 2) {
        let message = "Coercion from " + toExpression(schema) + " to " + toExpression(target) + " is not supported";
  ...
  while (current) {
    ...
    if (items !== undefined) {
      ...
      for (let idx = 0, idx_finish = items.length; idx < idx_finish; ++idx) {
        ...
        let reversed_inlinedLocation = item.inlinedLocation;
        let reversed = {
...
function internalCompile(schema, flag, defs) {
  ...
  let output$1 = parse(b, schema, input, "");
...
function jsonableValidation(output, parent, path, flag) {
  ...
  items.forEach(item => jsonableValidation(item.schema, output, path + ("[" + item.inlinedLocation + "]"), flag));
}
...
function parse$1(value) {
  ...
  if ($$typeof === "object") {
    ...
    if (Number.isNaN(value)) {
      let mut$1 = new Schema();
      mut$1.type = "nan";
      schema = mut$1;
    } else {
      let mut$2 = new Schema();
      mut$2.type = $$typeof;
      schema = mut$2;
    }
  } else {
    let mut$3 = new Schema();
    mut$3.type = $$typeof;
    schema = mut$3;
  }
  schema.const = value;
...
function internalRefine(schema, refiner) {
  return updateOutput(schema, mut => {
    ...
    mut.refiner = (b, input, selfSchema, path) => {
      let input$1 = prevRefiner !== undefined ? prevRefiner(b, input, selfSchema, path) : input;
...
function transform(schema, transformer) {
  return updateOutput(schema, mut => {
    ...
    let to = new Schema();
    mut.to = (to.type = "unknown", to.serializer = (b, input, selfSchema, path) => {
      let match = transformer(effectCtx(b, selfSchema, path));
...
function getItemCode(b, schema, input, output, deopt, path) {
  try {
    ...
    if (deopt) {
      let filterCode = typeFilterCode(bb, schema, input, path);
      bb.c = bb.c + filterCode;
    }
    let input$1 = {
      b: input.b,
      v: input.v,
  ...
  for (let idx = 0; idx <= lastIdx; ++idx) {
    ...
    switch (tag) {
      ...
      case "ref" :
        ...
        break;
      default:
        let key = tag === "instance" ? schema.class.name : tag;
        let arr = byKey[key];
        if (arr !== undefined) {
...
function nestedNone() {
  ...
  return {
    type: "object",
    serializer: (b, param, selfSchema, param$1) => ({
      b: b,
      v: _notVar,
...
function factory$1(item, unitOpt) {
  let unit = unitOpt !== undefined ? unitOpt : $$undefined;
...
function getWithDefault(schema, $$default) {
  return updateOutput(schema, mut => {
    ...
    if (anyOf !== undefined) {
      ...
      for (let idx = 0, idx_finish = anyOf.length; idx < idx_finish; ++idx) {
        let item = anyOf[idx];
        let itemOutput = getOutputSchema(item);
      ...
      if (newAnyOf.length === anyOf.length) {
        let message = toExpression(mut) + " doesn't have undefined case for default";
        throw new Error("[Sury] " + message);
      }
      mut.parser = (b, input, param, param$1) => {
        let inputVar = input.v(b);
        let tmp;
      ...
      if ($$default.TAG === "Value") {
        mut.default = Primitive_option.some($$default._0);
        return;
      } else {
        return;
      }
...
function factory$4(item, spaceOpt) {
  ...
  let to = copyWithoutCache(item);
  to.serializer = (b, input, selfSchema, param) => {
...
let schema$2 = new Schema();

schema$2.type = "number";

let schema$3 = new Schema();
...
function schemaRefiner(b, input, selfSchema, path) {
  ...
  for (let idx$1 = 0, idx_finish$1 = items.length; idx$1 < idx_finish$1; ++idx$1) {
    ...
    let path$1 = path + ("[" + inlinedLocation$1 + "]");
  ...
  if ((additionalItems !== "strip" || b.g.o & 32) && items.every(item => objectVal$1[item.inlinedLocation] === input[item.inlinedLocation])) {
    return input;
  } else {
  ...
  return (b, input, selfSchema, path) => {
    ...
    if (!isFlatten) {
      ...
      for (let idx = 0, idx_finish = items.length; idx < idx_finish; ++idx) {
        ...
        let itemInput = get(b, input, inlinedLocation);
...
function option(item) {
  return factory$1(item, $$undefined);
}
...
function toIntSchema(jsonSchema) {
  ...
  if (minimum !== undefined) {
    schema$4 = intMin(schema$1, minimum | 0, undefined);
  } else {
    let exclusiveMinimum = jsonSchema.exclusiveMinimum;
    schema$4 = exclusiveMinimum !== undefined ? intMin(schema$1, exclusiveMinimum + 1 | 0, undefined) : schema$1;
  }
  let maximum = jsonSchema.maximum;
  if (maximum !== undefined) {
    return intMax(schema$4, maximum | 0, undefined);
  }
  let exclusiveMinimum$1 = jsonSchema.exclusiveMinimum;
  if (exclusiveMinimum$1 !== undefined) {
    return intMax(schema$4, exclusiveMinimum$1 - 1 | 0, undefined);
  } else {
    return schema$4;
  }
}
...
function fromJSONSchema(jsonSchema) {
  ...
  if (jsonSchema.nullable) {
    schema$4 = factory$6(fromJSONSchema(Object.assign({}, jsonSchema, {
      nullable: false
    })));
    ...
    if (type_$1 === "object") {
      ...
      if (properties !== undefined) {
        ...
        schema$4 = additionalProperties !== undefined && Primitive_option.valFromOption(additionalProperties) === false ? strict(schema$5) : schema$5;
      } else {
      ...
      if (items !== undefined) {
        ...
        if (single.TAG === "Single") {
          schema$6 = factory$2(definitionToSchema$1(single._0));
        } else {
          let array = single._0;
          schema$6 = tuple(s => array.map((d, idx) => s.item(idx, definitionToSchema$1(d))));
        }
      } else {
        schema$6 = factory$2(json);
      }
      let min = jsonSchema.minItems;
      let schema$7 = min !== undefined ? arrayMinLength(schema$6, min, undefined) : schema$6;
      let max = jsonSchema.maxItems;
      schema$4 = max !== undefined ? arrayMaxLength(schema$7, max, undefined) : schema$7;
    } else {
      exit$1 = 2;
    }
  } else {
    ...
    if (definitions$1 !== undefined) {
      ...
      if (definitions$2 !== undefined) {
        ...
        if (not !== undefined) {
          ...
          if ($$const !== undefined) {
            schema$4 = parse$1($$const);
          } else if (type_ !== undefined) {
            let type_$2 = Primitive_option.valFromOption(type_);
            let exit$2 = 0;
            let exit$3 = 0;
            if (Array.isArray(type_$2)) {
              schema$4 = factory(type_$2.map(type_ => fromJSONSchema(Object.assign({}, jsonSchema, {
                type: Primitive_option.some(type_)
              }))));
            } else if (type_$2 === "string") {
              let p = jsonSchema.pattern;
              let schema$8 = p !== undefined ? pattern(schema, new RegExp(p), undefined) : schema;
              let minLength = jsonSchema.minLength;
              let schema$9 = minLength !== undefined ? stringMinLength(schema$8, minLength, undefined) : schema$8;
              let maxLength = jsonSchema.maxLength;
              let schema$10 = maxLength !== undefined ? stringMaxLength(schema$9, maxLength, undefined) : schema$9;
              switch (jsonSchema.format) {
                case "date-time" :
                  schema$4 = datetime(schema$10, undefined);
                  break;
                case "email" :
                  schema$4 = email(schema$10, undefined);
                  break;
                case "uri" :
                  schema$4 = url(schema$10, undefined);
                  break;
                case "uuid" :
                  schema$4 = uuid(schema$10, undefined);
                  break;
                default:
                  schema$4 = schema$10;
              }
            } else if (type_$2 === "integer" || jsonSchema.format === "int64" && type_$2 === "number") {
              schema$4 = toIntSchema(jsonSchema);
            } else {
              exit$3 = 4;
            }
            if (exit$3 === 4) {
            ...
            if (exit$2 === 3) {
              if (type_$2 === "number") {
                ...
                let schema$11;
                if (minimum !== undefined) {
                  schema$11 = floatMin(schema$2, minimum, undefined);
                } else {
                  let exclusiveMinimum = jsonSchema.exclusiveMinimum;
                  schema$11 = exclusiveMinimum !== undefined ? floatMin(schema$2, exclusiveMinimum + 1, undefined) : schema$2;
                }
                let maximum = jsonSchema.maximum;
                if (maximum !== undefined) {
                  schema$4 = floatMax(schema$11, maximum, undefined);
                } else {
                  let exclusiveMinimum$1 = jsonSchema.exclusiveMinimum;
                  schema$4 = exclusiveMinimum$1 !== undefined ? floatMax(schema$11, exclusiveMinimum$1 - 1, undefined) : schema$11;
                }
              } else if (type_$2 === "boolean") {
                schema$4 = bool;
              } else if (type_$2 === "null") {
                schema$4 = js_schema(null);
              } else {
                exit = 1;
              }
            }
    ...
    if (if_ !== undefined) {
      ...
      if (then !== undefined) {
        ...
        if (else_ !== undefined) {
          ...
          schema$4 = refine(json, param => (data => {
            let passed;
            try {
