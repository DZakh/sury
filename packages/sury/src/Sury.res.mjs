...
function toExpression(schema) {
  ...
  if (format !== undefined) {
    ...
    let propsToUse = properties !== undefined ? properties : (
        to !== undefined ? to.properties : undefined
      );
    if (propsToUse === undefined) {
      return "unknown[][]";
    }
    let keys = Object.keys(propsToUse);
    return "[" + keys.map(key => {
      let propSchema = propsToUse[key];
  ...
  switch (tag) {
    ...
    case "object" :
      let match = schema.to;
      if (match !== undefined) {
        let match$1 = match.format;
        if (match$1 === "compactColumns") {
          let properties$1 = schema.properties;
          let locations = Object.keys(properties$1);
          if (locations.length === 0) {
            return "{}[]";
          } else {
            return "{ " + locations.map(location => location + ": " + toExpression(properties$1[location]) + ";").join(" ") + " }[]";
          }
        }
        
      }
      let additionalItems = schema.additionalItems;
      let properties$2 = schema.properties;
      let locations$1 = Object.keys(properties$2);
      if (locations$1.length === 0) {
        ...
        return "{ " + locations$1.map(location => location + ": " + toExpression(properties$2[location]) + ";").join(" ") + " }";
      }
    default:
      if (schema.b) {
        return tag;
...
function objectDecoder(unknownInput, param) {
  let isUnion = unknownInput.u;
  let expectedSchema = unknownInput.e;
  let match = expectedSchema.to;
  let exit = 0;
  if (match !== undefined) {
    let match$1 = match.format;
    if (match$1 !== undefined) {
      if (match$1 === "compactColumns") {
        let output = refine(unknownInput, undefined, undefined, undefined);
        output.k = false;
        return output;
      }
      exit = 1;
  ...
  if (exit === 1) {
    ...
    if (itemSchema === "strip" || itemSchema === "strict") {
      ...
      return asyncVal(output$1, "new Promise((" + resolveVar + "," + rejectVar + ")=>{let " + counterVar + "=Object.keys(" + outputVar + ").length;for(let " + keyVar + " in " + outputVar + "){" + outputVar + "[" + keyVar + "].then(" + asyncParseResultVar + "=>{" + outputVar + "[" + keyVar + "]=" + asyncParseResultVar + ";if(" + counterVar + "--===1){" + resolveVar + "(" + outputVar + ")}}," + rejectVar + ")}})");
    }
    if (exit$1 === 2) {
...
function compactColumns(inputSchema) {
  ...
  mut.serializer = compactColumnsEncoder;
  return mut;
}
