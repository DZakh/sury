...
function failWithErrorMessage(key, defaultMessage) {
  return input => {
    ...
    if (em !== undefined) {
      let m = em[key];
      message = m !== undefined ? m : defaultMessage;
    } else {
...
function enableEmail() {
  ...
  email.refiner = input => [{
      ...
      f: failWithErrorMessage("format", message)
    }];
}
...
function meta(schema, data) {
  ...
  if (em !== undefined) {
    let d = mut.errorMessage;
    let existing = d !== undefined ? copy(d) : ({});
    Js_dict.entries(em).forEach(param => {
      existing[param[0]] = param[1];
    });
    mut.errorMessage = existing;
  }
  return mut;
}
