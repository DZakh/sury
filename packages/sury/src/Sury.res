...
let enableEmail = () => {
  if email->Obj.magic->Js.Dict.unsafeGet(shakenRef)->Obj.magic {
    ...
    email.errorMessages = Some(Js.Dict.fromArray([("format", "Invalid email address")]))
    email.refiner = Some(
      (~input) => {
...
let enableUuid = () => {
  if uuid->Obj.magic->Js.Dict.unsafeGet(shakenRef)->Obj.magic {
    ...
    uuid.errorMessages = Some(Js.Dict.fromArray([("format", "Invalid UUID")]))
    uuid.refiner = Some(
      (~input) => {
...
let enableCuid = () => {
  if cuid->Obj.magic->Js.Dict.unsafeGet(shakenRef)->Obj.magic {
    ...
    cuid.errorMessages = Some(Js.Dict.fromArray([("format", "Invalid CUID")]))
    cuid.refiner = Some(
      (~input) => {
...
let enableUrl = () => {
  if url->Obj.magic->Js.Dict.unsafeGet(shakenRef)->Obj.magic {
    ...
    url.format = Some(Url)
    url.errorMessages = Some(Js.Dict.fromArray([("format", "Invalid url")]))
    url.refiner = Some(
      (~input) => {
