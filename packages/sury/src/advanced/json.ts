...
// `S.protobufValue`: `S.json` as a `google.protobuf.Value` field, with `Struct`
// and `ListValue` for its objects and arrays. A Value with no kind set reads
// as `null`, the kind its `null_value` names. Seen again, a Value merges the
// way a oneof member does: a Struct into a Struct and a list onto a list,
...
const readValue = (r: Reader, d: number, prev: unknown = null): unknown => {
  ...
  while (r.pos < r.limit) {
    ...
    if (tag === 8) {
      ...
      // A Struct's fields are a `map<string, Value>`: a key repeated later
      // replaces the entry, and one named `__proto__` stays an own property.
      const struct = (value !== null && typeof value === "object" && !Array.isArray(value) ? value : {}) as Record<string, unknown>;
      const end = r.sub();
      while (r.pos < r.limit) {
        const field = r.tag();
        if (field !== 10) {
        ...
        while (r.pos < r.limit) {
          ...
          if (inner === 10) key = r.string();
            ...
            const at = r.sub();
            item = readValue(r, d + 2, item);
            r.limit = at;
          } else r.skipTag(inner);
        }
        r.limit = entry;
        if (key === "__proto__") Object.defineProperty(struct, key, { value: item, enumerable: true, writable: true, configurable: true });
      ...
      while (r.pos < r.limit) {
        ...
        if (field === 10) {
          const at = r.sub();
          list.push(readValue(r, d + 2));
          r.limit = at;
        } else r.skipTag(field);
      }
      r.limit = end;
      value = list;
    } else r.skipTag(tag);
...
// Writes one value inside a length its caller frames, as the message writer of
// a field does.
...
export const protobufValue: Internal = /* @__PURE__ */ (() => {
  const s = copySchema(json);
  s.protobufCodec = {
