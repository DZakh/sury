...
import { getCarrierSchema } from "../parse";
...
// absent arm, a coercion or a jsonString rejects it itself. A chain decides by
// what it ends in - the links between convert the text, and the unset var
// reaches none of them.
const rejectsUnset = (target: Internal): boolean => {
  const out = getCarrierSchema(target);
  return (tagFlags[out.type]! & 2) !== 0 && out.format !== "json" && !isAbsent(out);
};
