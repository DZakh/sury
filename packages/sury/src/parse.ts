...
export const throwTail: Tail = (input, code, out, isAsync, flag, hasDefs) => {
  if (flag & 1024) {
    ...
    //
    // The JS Result tail (`errResult`, operations.ts) emits this same issue
    // shape, because a Result IS a Standard Schema result - change one and the
    // other has to follow. `tests/operations_test.ts` compares the two.
