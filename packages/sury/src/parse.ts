...
import { B_scope, B_embedInvalidInput, B_joinCode, B_inlineConst, B_markOutput, B_merge, B_next, B_operationArg, B_refine, B_unsupportedDecode, Builder, Encoder, failInvalidType, noopOperation, operationArgVar } from "./builder";
...
export const compileDecoder = (
  ...
  const output = parse(input);
  // The single join: everything upstream builds a chunk tree; nothing is
  // frozen until here.
  const code = B_joinCode(B_merge(output));
