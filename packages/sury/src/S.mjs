...
var blob = /* @__PURE__ */ initSchema(instanceTag, (s2) => {
  s2.class = globalThis.Blob;
  s2.decoder = instanceDecoder;
});
var file = /* @__PURE__ */ initSchema(instanceTag, (s2) => {
  s2.class = globalThis.File;
  s2.decoder = instanceDecoder;
});
...
var assertSizeBound = (fnName, schema, value) => {
  const proto = schema.class?.prototype;
...
var withSizeBounds = (schema, base) => {
...
var setBoundExpression = (mut, schema, render) => {
  if (schema.bounds === U) {
    const base = schema.expression;
    mut.expression = (s2) => render(s2, base !== U ? base(s2) : inputExpression(s2, true));
  }
};
...
var asBound = (schema, key, bit, value, render) => {
  const mut = { ...schema, bounds: bit };
  mut[key] = value;
  setBoundExpression(mut, schema, render);
  return mut;
};
var assertLower = (schema, value, exclusive) => {
  ...
  if (strict2 !== U && bound >= strict2) {
    conflict(asBound(schema, key, bit, value, withBounds), asBound(schema, "exclusiveMaximum", 8, strict2, withBounds));
  }
};
var assertUpper = (schema, value, exclusive) => {
  ...
  if (strict2 !== U && bound <= strict2) {
    conflict(asBound(schema, key, bit, value, withBounds), asBound(schema, "exclusiveMinimum", 4, strict2, withBounds));
  }
};
var assertSize = (schema, value, upper, render) => {
...
var minLength = /* @__NO_SIDE_EFFECTS__ */ (schema, length2, maybeMessage) => {
  ...
  assertSize(schema, length2, false, withBounds);
...
var maxLength = /* @__NO_SIDE_EFFECTS__ */ (schema, length2, maybeMessage) => {
  ...
  assertSize(schema, length2, true, withBounds);
...
var assertSizeRange = (schema, value, upper) => {
...
var minSize = /* @__NO_SIDE_EFFECTS__ */ (schema, size2, maybeMessage) => {
  assertSizeBound("minSize", schema, size2);
  assertSizeRange(schema, size2, false);
  if (!narrowsSize(schema.minSize, size2, false)) return schema;
  return internalRefine(schema, (mut) => {
    ...
    return (_input) => {
      return [
        {
          c: (inputVar) => `${inputVar}.size>${size2 - 1}`,
...
var maxSize = /* @__NO_SIDE_EFFECTS__ */ (schema, size2, maybeMessage) => {
  assertSizeBound("maxSize", schema, size2);
  assertSizeRange(schema, size2, true);
  if (!narrowsSize(schema.maxSize, size2, true)) return schema;
  return internalRefine(schema, (mut) => {
    setBoundExpression(mut, schema, withSizeBounds);
    mut.bounds = (schema.bounds ?? 0) | 2;
    mut.maxSize = size2;
...
var size = /* @__NO_SIDE_EFFECTS__ */ (schema, size2, maybeMessage) => {
  assertSizeBound("size", schema, size2);
  assertSizeRange(schema, size2, false);
  assertSizeRange(schema, size2, true);
  if (schema.minSize === size2 && schema.maxSize === size2) return schema;
  return internalRefine(schema, (mut) => {
    setBoundExpression(mut, schema, withSizeBounds);
    mut.bounds = (schema.bounds ?? 0) | 3;
    mut.minSize = size2;
    ...
    return (_input) => {
      return [
        {
          c: (inputVar) => `${inputVar}.size===${size2}`,
          f: B_failWithErrorMessage("minSize")
        }
      ];
    };
