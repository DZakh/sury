...
// Encodes Uint8Array to a base64url string
S.uint8Array.with(S.to, S.base64url);
// Base64 of utf-8 text, both directions
S.base64.with(S.to, S.uint8Array).with(S.to, S.string);
```

`S.uint8Array.with(S.to, S.string)` decodes UTF-8; the base64 formats are the
only string targets that don't, since they re-encode the same bytes instead of
interpreting them as text. The format is validated before anything is decoded,
