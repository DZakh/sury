...
```ts
S.toJSONSchema(S.ulid); // { type: "string", pattern: "^[0-7][0-9A-HJKMNP-TV-Za-hjkmnp-tv-z]{25}$" }
S.toJSONSchema(S.uuidv7); // { type: "string", format: "uuid", pattern: "…-7[0-9a-fA-F]{3}-…" }
S.toJSONSchema(S.httpUrl); // { type: "string", format: "uri", pattern: "^[hH][tT][tT][pP][sS]?:" }
```
