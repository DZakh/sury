...
Same three words, different things. Let's fix that.
...
Now put yourself on the other side. You maintain a tool that wants "give me your schema". You either write an adapter per library and keep them all updated forever, or you pick one library and tell everyone else to convert by hand. Almost everyone picked Zod.

## Before the standard, people got creative

JSON Schema's classic use case is generating OpenAPI, and tRPC needed that. With no way to accept a schema from any library, [its OpenAPI generator](https://trpc.io/docs/openapi) went around the runtime entirely:

> The generator statically analyses your router's TypeScript types — it never executes your code.

It reads your API shape out of the type system with the compiler, instead of asking your schemas what they are. Even the descriptions:

> Zod `.describe()` calls and JSDoc comments on types, routers, and procedures, all become `description` fields in the spec.

`.describe()` is a runtime call, so it has to survive into the TypeScript type for a static analyzer to pick it back up. That's a lot of machinery to answer a question every schema in the router could have answered directly. 👀
...
`target` is required, so the dialect is the caller's choice - `draft-2020-12`, `draft-07` or `openapi-3.0` - instead of a per-library default you have to go look up.
...
`schema` there is yours - Zod, Valibot or Sury, and nothing else in the snippet changes. Same request on the wire, and `output_parsed` typed from whichever you passed. Internally the SDK just calls `schema["~standard"].jsonSchema?.input({ target: "draft-07" })`.

Compare that with `zodTextFormat`: the same helper, with one library's name baked into it.
...
## What you get over a plain JSON Schema

**Your users pick their own library.** That's the entire point, and it's worth more to them than it costs you.

**Types come along.** `~standard.types` carries the inferred Input and Output, so your tool's return value is typed from the user's schema without a single generic of your own.
...
Standard JSON Schema is the way out, in three steps.
...
Nothing existing breaks, and every schema library starts working with your tool at once. For most tools this is the whole job.
...
So call that instead of running your own validator, and hand back its errors. Your users get messages from the library they already know, and you stop checking the same data twice.
...
If you maintain a schema library, implement it. If you maintain a tool, Level 1 is one branch and buys you every library at once. And if you're just picking a library, check whether yours does it yet.
